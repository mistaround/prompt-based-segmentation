#!/usr/bin/env python3
"""Benchmark TinySAM on the shared coco128-seg protocol.
...
def build(checkpoint: str) -> SamStyleAdapter:
    ...
    return SamStyleAdapter(
        name="TinySAM",
        variant=f"vit_t ({pathlib.Path(checkpoint).name})",
        model=model,
        predictor=SamPredictor(model),
        # TinySAM's predict() takes no multimask argument -- it always returns
        # three candidates, so both prompt types pick by predicted score.
        multimask_kw=None,
    )
